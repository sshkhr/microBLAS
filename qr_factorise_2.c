...
void qrhh1(int m, int n,  double** A)
{
	...
	double** reflector = (double **)calloc(m,sizeof(double*));
	for(j = 0; j < m; j++)
		reflector[j] = (double *)calloc(m,sizeof(double));

	double** sub_matrix = (double **)malloc(m*sizeof(double*));
	for(j = 0; j < m; j++)
		sub_matrix[j] = (double *)malloc(n*sizeof(double));
